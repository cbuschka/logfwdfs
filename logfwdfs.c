...
static struct dentry *logfwdfs_mount(struct file_system_type *type, int flags, char const *dev, void *data);
...
static void __exit logfwdfs_cleanup(void)
{
    ...
    printk(KERN_INFO "logfwdfs: logfwdfs unregistered.\n");
...
static int logfwdfs_fill_sb(struct super_block *sb, void *data, int silent)
  {
      ...
      sb->s_magic = LOGFWDFS_MAGIC;
      sb->s_op = &logfwdfs_super_ops;
      ...
      root->i_ino = 0;
      root->i_sb = sb;
      ktime_get_coarse_ts64(&(root->i_atime));
      ktime_get_coarse_ts64(&(root->i_mtime));
      ktime_get_coarse_ts64(&(root->i_ctime));
...
static struct dentry *logfwdfs_mount(struct file_system_type *type, int flags,
  ...
  {
      struct dentry *const entry = mount_nodev(type, flags, data, logfwdfs_fill_sb);
      if (IS_ERR(entry))
